...
4. Reads each client's `data/services.json` → regenerates `docker-compose.yml` + `gateway/Caddyfile`
5. Rsyncs `prod_setup/client_server/` → client VM, `prod_setup/main_server/` → main VM
6. SSH: `docker compose pull && docker compose up -d` on each VM
7. Commits any synced data changes and tags the deploy (`deploy-YYYY-MM-DD`)
...
**Gateway Caddyfile** (`prod_setup/client_server/<client>/gateway/Caddyfile`) —
per-client, uses stock `caddy:2.10.2-alpine` image with volume-mounted config:
...
The gateway Caddyfile is regenerated by `deploy.sh` from `data/services.json` —
only enabled services get routes. New clients start with a minimal Caddyfile
(admin + prompt-composer only).
...
- Site chat: browser → client router → gateway `/site/*` → prompt-composer `/ask`
