(function() {
  ...
  const css = `
    ...
    .chat-msg.user { background: #fff; color: #000; align-self: ${isRTL ? 'flex-start' : 'flex-end'}; text-align: ${isRTL ? 'left' : 'right'}; border: 1px solid #ddd; ${isRTL ? 'margin-left: 12px;' : 'margin-right: 12px;'} }
    .chat-msg.bot { background: #f5f5f5; color: #000; align-self: ${isRTL ? 'flex-end' : 'flex-start'}; border: 1px solid #ddd; ${isRTL ? 'margin-right: 12px;' : 'margin-left: 12px;'} }
    .chat-tail { position: absolute; top: 16px; width: 12px; height: 12px; }
    .chat-tail.user { ${isRTL ? 'left: -12px;' : 'right: -12px;'} }
    .chat-tail.bot { ${isRTL ? 'right: -12px;' : 'left: -12px;'} }
  ...
  setTimeout(() => {
    ...
    const addMsg = (content, role) => {
      ...
      if (role === 'user') {
        path.setAttribute('d', isRTL ? 'M 12 0 L 0 6 L 12 12 Z' : 'M 0 0 L 12 6 L 0 12 Z');
        path.setAttribute('fill', '#fff');
      } else {
        path.setAttribute('d', isRTL ? 'M 0 0 L 12 6 L 0 12 Z' : 'M 12 0 L 0 6 L 12 12 Z');
