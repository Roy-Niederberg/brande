(function() {
  ...
  const css = `
    #chat-widget { position: fixed; bottom: 20px; right: 20px; width: 380px; max-width: 90vw; z-index: 9999; font-family: Arial, sans-serif; }
    ...
    .chat-msg.user { background: #fff; color: #000; align-self: flex-end; text-align: right; border: 1px solid #ddd; margin-right: 12px; }
    .chat-msg.bot { background: #f5f5f5; color: #000; align-self: flex-start; border: 1px solid #ddd; margin-left: 12px; }
    .chat-tail { position: absolute; top: 16px; width: 12px; height: 12px; }
    .chat-tail.user { right: -12px; }
    .chat-tail.bot { left: -12px; }
    #chat-input-area { display: flex; gap: 8px; position: relative; z-index: 10; }
