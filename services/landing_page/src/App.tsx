    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[var(--bg)]/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <a href="#features" className={`text-sm font-medium hover:opacity-70 transition-opacity ${isScrolled ? 'text-[var(--text)]' : 'text-white/90'}`}>{t('nav.features')}</a>
          <a href="#how-it-works" className={`text-sm font-medium hover:opacity-70 transition-opacity ${isScrolled ? 'text-[var(--text)]' : 'text-white/90'}`}>{t('nav.howItWorks')}</a>
          <button
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${isScrolled ? 'text-[var(--text)] hover:bg-[var(--primary)]/10' : 'text-white/80 hover:text-white'}`}
          >
            {locale === 'en' ? 'HE' : 'EN'}
          </button>
        ...
        <div className="flex md:hidden items-center gap-2">
          <button
            onClick={toggleLocale}
            className={`px-2.5 py-1 rounded-full text-xs font-semibold ${isScrolled ? 'text-[var(--text)]' : 'text-white/80'}`}
          >
            {locale === 'en' ? 'HE' : 'EN'}
          </button>
          <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[var(--bg)] shadow-lg border-t border-[var(--border)] p-6 flex flex-col gap-4">
          <a href="#features" className="text-[var(--text)] font-medium" onClick={() => setMobileMenuOpen(false)}>{t('nav.features')}</a>
          <a href="#how-it-works" className="text-[var(--text)] font-medium" onClick={() => setMobileMenuOpen(false)}>{t('nav.howItWorks')}</a>
  ...
  return (
    <section className="relative h-screen min-h-[600px] flex items-end pb-16 md:pb-24 overflow-hidden">
      ...
      <div className="relative z-20 w-full max-w-[1600px] mx-auto px-8 md:px-16 flex flex-col md:flex-row justify-between items-end gap-12">
        ...
        >
          <h1 className={`text-5xl md:text-6xl lg:text-[5rem] text-white tracking-tight leading-[1.05] ${locale === 'he' ? 'font-normal' : 'font-light'}`}>
            {t('hero.title1')} <br />
            {t('hero.title2')}
          </h1>
        </motion.div>
          ...
          {[0, 1, 2].map((i) => (
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-bold text-white mb-3">{t(`features.${i}.title`)}</h3>
                <p className="text-[#FAF4E8]/80 leading-relaxed">{t(`features.${i}.desc`)}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-6 leading-tight">
              ...
              <span className="text-[#85C1E9]/60">{t('howItWorks.heading2')}</span>
            </h2>
          ...
          >
            <div className="aspect-square md:aspect-[4/3] rounded-3xl overflow-hidden relative">
              ...
              <div className="absolute bottom-8 left-8 right-8 bg-[#2A4359]/80 backdrop-blur-md border border-[#85C1E9]/20 rounded-2xl p-6">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <div className="text-sm font-medium text-[#FAF4E8]">{t('howItWorks.qabuLabel')}</div>
                    <div className="text-xs text-[#85C1E9]">{t('howItWorks.activeNow')}</div>
                  </div>
                </div>
            ...
            {/* Decorative elements */}
            <div className="absolute -top-6 -end-6 w-24 h-24 bg-[#85C1E9]/20 rounded-full blur-2xl" />
            <div className="absolute -bottom-10 -start-10 w-32 h-32 bg-[#1B4F72]/30 rounded-full blur-2xl" />
          </motion.div>
        </div>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
          ...
          <div className="flex gap-6 text-sm">
            <a href="/privacy" className="hover:text-[#FAF4E8] transition-colors">{t('footer.privacy')}</a>
            <a href="/terms" className="hover:text-[#FAF4E8] transition-colors">{t('footer.terms')}</a>
          </div>
        </div>

        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-xs border-t border-[#2A4359]/50 pt-8">
          <p>&copy; {new Date().getFullYear()} Qab&ucirc;. {t('footer.rights')}</p>
          <p className="text-[#FAF4E8]/30">
            {t('footer.videoBy')} <a href="https://mixkit.co" target="_blank" rel="noopener noreferrer" className="text-[#FAF4E8]/40 hover:text-[#85C1E9] underline decoration-[#2A4359] underline-offset-2">Mixkit</a>.
            {' '}{t('footer.imagesFrom')} <a href="https://unsplash.com" target="_blank" rel="noopener noreferrer" className="text-[#FAF4E8]/40 hover:text-[#85C1E9] underline decoration-[#2A4359] underline-offset-2">Unsplash</a>.
          </p>
        </div>
      </div>
    </footer>
