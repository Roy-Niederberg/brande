 ...
 * Fetches client-config.json and sets up the page accordingly
 */
(async function() {
    ...
    if (config.font?.family) {
        ...
        if (titleEl) {
            titleEl.style.fontFamily = `'${config.font.family}', sans-serif`;
        }
    }
    ...
    if (config.overlayTitle) {
        ...
        if (config.font?.family) {
            titleDiv.style.fontFamily = `'${config.font.family}', sans-serif`;
        }
        titleDiv.textContent = config.overlayTitle;
    ...
    if (config.socialLinks?.length) {
        ...
        for (const link of config.socialLinks) {
            ...
            a.href = link.url;
            a.target = '_blank';
            a.title = link.icon.charAt(0).toUpperCase() + link.icon.slice(1);
            a.innerHTML = `<svg width="40" height="40" viewBox="0 0 24 24" fill="white">${icons[link.icon] || ''}</svg>`;
